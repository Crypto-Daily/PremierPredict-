  ...
  try {
    ...
    const response = await fetch("https://api.paystack.co/transaction/initialize", {
      ...
      body: JSON.stringify({
        ...
        amount,
        callback_url: `https://crypto-daily.github.io/PremierPredict-/success.html?ticket=${ticketId}`, // redirect to frontend with ticket
        metadata: { phone, match, ticketId }, // save ticket inside Paystack metadata
      }),
    });
...
app.get("/verify-payment/:reference", async (req, res) => {
  try {
    ...
    if (data.status && data.data.status === "success") {
      res.json({
        success: true,
        ticketId: data.data.metadata.ticketId, // return stored ticket
        phone: data.data.metadata.phone,
        match: data.data.metadata.match,
      });
    } else {
